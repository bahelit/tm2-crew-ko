...
| A second board overlapping the left HUD | PyPlanet's **Live Rankings** widget sits around x -124.75 and lands on the match HUD (x -159..-113) during the BOTN/cup warm-ups and the cup match. Disabled in `settings/apps.py` on 2026-08-08 — check the `live_rankings` line is still commented out, and restart (an `APPS` change is only read at boot). Older builds also drew a blue cup standings box; that was retired the same day. A manialink stays painted until something replaces it, so a client connected across the change may keep showing either until it reconnects. |
